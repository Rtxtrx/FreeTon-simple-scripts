...
# (C) Sergey Tyurin  2020-09-30 12:00:00
...
NANO_AMOUNT=`tonos-cli convert tokens $TRANSF_AMOUNT| grep "[0-9]"`
...
SRC_ACCOUNT=`cat ./addresses/${SRC_NAME}.addr`
DST_ACCOUNT=`cat ./addresses/${DST_NAME}.addr`
SRC_KEY_FILE="./addresses/${1}.keys.json"

echo "Check SRC $SRC_NAME account.."
SRC_BALANCE_INFO=`tonos-cli account $SRC_ACCOUNT || echo "ERROR get balance" && exit 0`
...
DST_BALANCE_INFO=`tonos-cli account $DST_ACCOUNT || echo "ERROR get balance" && exit 0`
...
for i in $(seq ${TONOS_CLI_SEND_ATTEMPTS}); do
    ...
    if ! "tonos-cli" call "${SRC_ACCOUNT}" submitTransaction \
        "{\"dest\":\"${DST_ACCOUNT}\",\"value\":\"${NANO_AMOUNT}\",\"bounce\":$BOUNCE,\"allBalance\":false,\"payload\":\"\"}" \
        --abi "./addresses/SafeMultisigWallet.abi.json" \
